...
namespace LibraryIS
{
    public partial class DelPost : Form
    {
        int id;
        ...
        private void deleteposst_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (id != 0)
                {
                    ...
                    logcmd.Parameters.Add("?umes", MySqlDbType.VarChar).Value = String.Format("Удлалил должность с id: {0}", id);
                    logcmd.ExecuteNonQuery();
                    conn.Close();
                    MessageBox.Show(String.Format("Должность с id:{0} удалена из базы.", id));
                    id = 0;
                    dataGridView1.Rows.Clear();
                    dataGridView1.Refresh();
                    LoadData();
